...
namespace dcq::algorithm {
    ...
    bool update_M(int iy, int ix, dcq::Parameters &params, dcq::Kernels &kernels, torch::Tensor &p);

    bool update_M(int iy, int ix, float *M_data, float *Y_data, float *p_data, float *bii_data,
                  int ks, int h, int w, int c, int K);

    void update_p(int iy, int ix, dcq::Parameters &params, torch::Tensor &a, torch::Tensor &b0, torch::Tensor &p);
